...
各位老师好，我是来自 张老师 课题组的周予恺。今天非常感谢各位老师在百忙之中参加我的中期答辩。

本次汇报将围绕我目前正在进行的研究工作展开，主题是基于流方程的量子动力学模拟方法的优化与GPU实现。该工作聚焦于突破当前多体量子系统动力学模拟中的规模瓶颈与算法效率问题，尝试引入扰乱变换与GPU并行计算策略提升传统 CUT 方法的可扩展性。

# p2 研究背景

各位老师，我先简单介绍一下这个工作的背景。

在当前量子物理和量子计算的研究中，我们经常需要研究量子多体系统在时间上的演化行为，比如一个系统从初态演化到某个远期态，它的能量是怎么转移的、信息是怎么扩散的等等。
...
- 张量网络方法，比如 MPS（矩阵乘积态）和 PEPS（投影纠缠对态），这类方法适合处理一维或二维系统的基态，也可以做一些短时间的动力学模拟。但如果系统中出现大量纠缠，张量网络的压缩效率就会迅速下降，计算开销也会变得很大，这被称为纠缠障碍。
    - 对于一个 $L$ 个粒子的系统，如果用传统方法表示整个波函数，需要 $2^L$ 个复数，非常庞大。而 MPS 的做法是：把这个波函数写成一系列小矩阵相乘的形式
- 量子蒙特卡罗，也叫 QMC，是一种统计抽样的方法，适合做平衡态性质的计算。但它在非平衡情况下，比如模拟系统如何随着时间演化，会受到符号问题的限制，这种情况下模拟结果的不确定性会迅速上升，甚至完全失效。
...
本研究考虑的是一个更一般的多体费米子系统，而不是具体限制于自旋链模型。

系统的哈密顿量由两个主要部分构成：
- 一个是**二体项**，描述粒子从一个格点跃迁到另一个格点，体现了系统的动能；
- 另一个是**四体相互作用项**，描述多个粒子之间的相互作用。
...
这里的符号 $: \cdots :$ 表示“正规序”，也就是按照一定顺序排列湮灭和产生算符，以消除真空态贡献。

- $c^\dagger_i$ 是在第 $i$ 个格点上创造一个费米子；
- $c_j$ 是在第 $j$ 个格点上湮灭一个费米子；
- $H^{(2)}_{ij}$ 和 $H^{(4)}_{ijkq}$ 是哈密顿量的系数矩阵，通常由具体模型或物理过程给定。

系统总共有 $L$ 个格点，也就是 $|\mathcal{L}| = L$，所以 Hilbert 空间维度指数增长，模拟变得非常困难。

这个模型在凝聚态、量子化学中广泛存在，也是 CUT 需要应对的通用输入格式。
...
通常我们处理的是二维、甚至三维格点系统，在这些系统中，自旋或费米子只与临近格点发生耦合，属于“局域哈密顿量”。
...
传统 CUT 方法中的一个瓶颈是 —— 物理量的计算需要变换回原始基底：
